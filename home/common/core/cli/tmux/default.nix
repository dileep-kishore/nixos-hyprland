{pkgs, ...}: {
  ...
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      # Session name
      session_name="#[fg=$thm_mauve,bg=$thm_base,italics] #S #[fg=$thm_mauve,bg=$thm_base,bold] $SEP";
      dir_section=" #[fg=$thm_blue,bg=$thm_base]#(~/.config/tmux/scripts/find_git_root.py #{pane_current_path}) #[fg=$thm_blue,bg=$thm_base,bold] $SEP";
